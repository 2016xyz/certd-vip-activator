export default {
  sys: {
    setting: {
      ...
      customAcmeManage: "自定义ACME管理",
      customAcmeManageHelper: "配置证书颁发机构（内置 + 自定义ACME）。内置颁发机构不允许修改和删除；自定义ACME适用于内网CA、企业CA等符合RFC 8555标准的ACME服务，配置后可在证书申请任务和ACME账号授权的颁发机构中选用",
      customAcmeName: "颁发机构名称",
      ...
      customAcmeBuiltIn: "内置",
      customAcmeTag: "自定义",
      customAcmeDeleteConfirmTitle: "删除自定义ACME",
      customAcmeDeleteConfirmContent: "删除「{title}」可能影响使用它的流水线申请证书，确认是否删除？",
      customAcmeSaveCheck: "自定义ACME的名称、标识、Directory URL均为必填",
      customAcmeSslProviderUnique: "自定义ACME的标识(sslProvider)不能重复",
      ...
      oauthAutoRedirectHelper: "是否自动跳转第三方登录（使用第一个已启用的第三方登录类型）\n注意：请务必先测试第三方登录功能正常后再开启。",
      oauthOnly: "仅使用第三方登录",
      oauthOnlyHelper: "是否仅使用第三方登录，关闭密码登录\n注意：请务必先测试第三方登录功能正常后再开启\n无法登录急救方法：服务重启后前10分钟内可通过 http://你的certd地址/#/login?oauthOnly=false， 使用密码登录处理配置问题",
